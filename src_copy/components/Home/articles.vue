    ...
    <ul class="article-list list-unstyled clearfix">
        <li v-for="article in articleList" class="article-item" :class="article.images.length>0?'have-img':''">
            <a v-if="article.images.length>0" v-link="{name:'article',params:{aid:article._id}}" class="wrap-img">
                <img :src="article.images[0].url+'-100*100'" />
            </a>
            <div>
                <p class="list-top">
                    <span class="time">
                        {{article.publish_time|customTime}}
                    </span>
                </p>
                <h4 class="title">
                    <a class="link-title" v-link="{name:'article',params:{aid:article._id}}">
                        {{article.title}}
                    </a>
                </h4>
        ...
        <li v-if="articleList.length<1" class="no-content">
            正在大力回车...
        </li>
    </ul>
</template>
