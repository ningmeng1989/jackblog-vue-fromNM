...
export const getArticleList=({dispatch},options,isAdd)=>{
    ...
    api.getFrontArticleList(options).then(response=>{
        ...
        isAdd
            ?dispatch(types.ADD_ARTICLE_LIST,{articleList:json.data,isMore:isMore})
            :dispatch(types.ARTICLE_LIST,{articleList:json.data,isMore:isMore})
    },response=>{
            dispatch(types.GET_ARTICLE_LIST_FAILURE);
    })
};

export const getArticleDetial=({dispatch},id,user)=>{
